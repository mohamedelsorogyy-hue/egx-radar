...
# استعلامات السوق. الرقم = وزن التأثير على محفظتك:
#   3 = بيحرّك السوق كله (فايدة، تضخم، عملة، حرب) — مفيش تنويع بيحميك منه
#   2 = بيحرّك قطاعات معينة (نفط، قمح، سياحة)
#   1 = خلفية مفيدة بس مش محرّك مباشر
MARKET_QUERIES = [
    # ── السياسة النقدية: أقوى محرّك للأسهم على الإطلاق ──────
    ("الفائدة", "البنك المركزي المصري سعر الفائدة", 3),
    ("الفائدة", "لجنة السياسة النقدية قرار الفائدة مصر", 3),
    ("الفائدة", "الفيدرالي الأمريكي سعر الفائدة", 3),

    # ── التضخم والعملة: بيحددوا القيمة الحقيقية لأرباحك ────
    ("التضخم", "معدل التضخم في مصر", 3),
    ("الدولار", "سعر الدولار مقابل الجنيه المصري", 3),
    ("الدولار", "احتياطي النقد الأجنبي مصر", 2),

    # ── جيوسياسي: بيضرب السوق فجأة ومن غير إنذار ───────────
    ("جيوسياسي", "التوترات في المنطقة تأثير الاقتصاد المصري", 3),
    ("جيوسياسي", "الحرب تأثير الأسواق الناشئة", 3),
    ("جيوسياسي", "قناة السويس إيرادات الملاحة", 2),

    # ── سلع عالمية: بتحدد تكاليف قطاعات كاملة ──────────────
    ("سلع", "أسعار النفط العالمية", 2),
    ("سلع", "أسعار القمح والغذاء عالمياً", 2),
    ("سلع", "أسعار الذهب عالمياً", 1),

    # ── الاقتصاد المصري ────────────────────────────────────
    ("مصر", "صندوق النقد الدولي مصر", 2),
    ("مصر", "الاستثمار الأجنبي في مصر", 2),
    ("مصر", "الناتج المحلي الإجمالي مصر النمو الاقتصادي", 1),

    # ── السوق نفسه ─────────────────────────────────────────
    ("السوق", "البورصة المصرية", 2),
    ("السوق", "مؤشر EGX30", 2),
    ("تنظيمي", "الرقابة المالية البورصة المصرية", 1),
]

IMPACT_LABEL = {3: "يحرّك السوق كله", 2: "يحرّك قطاعات", 1: "خلفية"}
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        ...
        for topic, impact, items in results:
            for it in items:
                # نفس الخبر بيتكرر بين الاستعلامات. لو اتكرر، بنسيبه
                # بأعلى وزن تأثير لقيناه — مش بأول واحد صادفناه.
                key = it["title"][:70]
                if key in seen:
                    prev = seen[key]
                    if impact > prev["impact"]:
                        prev["impact"] = impact
                        prev["impactLabel"] = IMPACT_LABEL[impact]
                        prev["topic"] = topic
                    continue
                it["topic"] = topic
                it["impact"] = impact
                it["impactLabel"] = IMPACT_LABEL[impact]
                seen[key] = it
                market.append(it)
    ...
    # حصة ثابتة لكل مستوى تأثير.
    # من غير الحصص دي، أخبار الفايدة والتضخم (وهي كتير) كانت بتاكل
    # القايمة كلها وأخبار السوق والقطاعات تختفي خالص.
    QUOTA = {3: 24, 2: 26, 1: 14}
    buckets = {3: [], 2: [], 1: []}
    for it in market:
        buckets.setdefault(it.get("impact", 1), []).append(it)
    market = []
    for level, cap in QUOTA.items():
        chunk = sorted(buckets.get(level, []),
                       key=lambda x: x.get("date") or "", reverse=True)
        market.extend(chunk[:cap])

    # الترتيب النهائي: الأهم أولاً مش الأحدث — خبر قرار فايدة من
    # امبارح أهم لمحفظتك من خبر عن جلسة اليوم.
    market.sort(key=lambda x: (x.get("impact", 1), x.get("date") or ""),
                reverse=True)

    tones = [m["tone"] for m in market]
    high = [m for m in market if m.get("impact") == 3]
    payload = {
        ...
        # الأخبار اللي بتحرّك السوق كله — بتتعرض في شريط منفصل فوق
        "critical": high[:6],
    }
